...
**`test/e2e/`** - compiles a `.jky` program to a real executable, runs it, and
...
A test is a `.jky` file. Lines starting with `// RUN:` are shell commands lit
...
Frontend example (`test/frontend/ir_function_call.jky`):
...
End-to-end example (`test/e2e/run_fib.jky`):
...
Drop a new `.jky` file in `test/frontend/` or `test/e2e/`. lit discovers it
