...
class CodeGenerator:
    ...
    def _declare_stdlib(self) -> None:
        ...
        decls = {
            ...
            'byovd_scan':             (self.i8p,   []),
            'byovd_driver_count':     (self.i64,   [self.i8p]),
            'byovd_driver_name':      (self.i8p,   [self.i8p, self.i64]),
            'byovd_driver_path':      (self.i8p,   [self.i8p, self.i64]),
            'byovd_driver_cve':       (self.i8p,   [self.i8p, self.i64]),
            'byovd_driver_risk':      (self.i8p,   [self.i8p, self.i64]),
            'byovd_load':             (self.i64,   [self.i8p]),
            'byovd_unload':           (self.voidT, []),
            'kernel_read':            (self.i8p,   [self.i64, self.i64]),
            'kernel_write':           (self.voidT, [self.i64, self.i64]),
            'kernel_base':            (self.i64,   []),
            'kernel_enum_callbacks':  (self.i8p,   []),
            'kernel_callback_count':  (self.i64,   [self.i8p]),
            'kernel_callback_addr':   (self.i64,   [self.i8p, self.i64]),
            'kernel_callback_module': (self.i8p,   [self.i8p, self.i64]),
            'kernel_patch_callback':  (self.i64,   [self.i64]),
